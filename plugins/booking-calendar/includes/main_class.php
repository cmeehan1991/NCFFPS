...
class wpdevart_Main {
	...
	**/
	...
	public function main_booking_calendar($id, $res_id = 0, $date='', $ajax = false, $selected = array(),$data = array(),$submit = "",$hours = array()){
		...
		if(isset($_POST["payment_submit_".$this->booking_id])) { 
		    $this->update_reservation($_POST,$this->booking_id);
            if(isset($_POST["payment_type_".$this->booking_id]) && $_POST["payment_type_".$this->booking_id] == "pay_in_cash"){ 
			
				wpdevart_bc_Library::wpdevart_redirect(esc_url($this->theme_option["redirect_url_successful"]));
				
			} elseif(isset($_POST["payment_type_".$this->booking_id]) && $_POST["payment_type_".$this->booking_id] == "payPal") {
		...
		if(isset($this->theme_option['payPal']) && isset($this->theme_option['payPal']) == "on"){
			$payments['payPal'] = "on";
		}	
		...
		if (!$ajax) {
			$booking_calendar .= "<div id='booking_calendar_main_container_" . $this->booking_id  . "' class='booking_calendar_main_container ".((isset($this->theme_option['hours_enabled']) && $this->theme_option['hours_enabled'] == "on") ? "hours_enabled" : "" )." ".((isset($this->widget) && $this->widget == true) ? "booking_widget" : "" )." ".((isset($this->theme_option['show_day_info_on_hover']) && $this->theme_option['show_day_info_on_hover'] == 'on') ? "show_day_info_on_hover" : "" )." ".((isset($this->theme_option['cal_animation_type']) && $this->theme_option['cal_animation_type'] != 'none') ? "animation_calendar" : "" )."' data-total='".$for_trarray["for_total"]."' data-price='".$for_trarray["for_price"]."' data-offset='".((isset($this->theme_option["scroll_offset"]) && $this->theme_option["scroll_offset"] != '') ? $this->theme_option["scroll_offset"] : 0)."' data-position='".((isset($this->theme_option["currency_pos"]) && $this->theme_option["currency_pos"] == 'before') ? "before" : "after")."' data-night='".((isset($this->theme_option["price_for_night"]) && $this->theme_option["price_for_night"] == 'on') ? "1" : "0")."' data-id='" . $id . "'>";
		...
		if (!$ajax) {
			if(isset($this->theme_option["type_days_selection"]) && $this->theme_option["type_days_selection"] == "multiple_days") {
				$booking_calendar .= "<div class='error_text_container div-for-clear'><span class='error_text'>".$for_trarray["for_error_multi"]."</span><span class='notice_text_close'><i class='fa fa-close'></i></span></div>";
		...
		if( isset($data["wpdevart-submit".$submit]) && count($payments)) {
			...
			foreach($payments as $key=>$payment){
				...
				$booking_calendar .= '<button type="button" class="wpdevart-payment-button ' . $button_cl . '" id="'.$key.'" name="wpdevart-payment'.$submit.'" data-id="'.$submit.'" data-resid="'.(isset($result)? $result['reservation_id'] : "").'" data-themeid="'.$this->ids["theme_id"].'">'.$button_content .'</button>';
			}
			$booking_calendar .= '<div class="wpdevart_order_wrap"></div>';
			$booking_calendar .= '<div class="wpdevart_order_content" id="wpdevart_booking_form_'.$id.'"></div>';
	...
	public function main_payment_ajax(){
		...
		$text_for = $this->text_for_tr();
		foreach($text_for as $key => $for) {
			if(isset($address['theme_option']['use_mo']) && $address['theme_option']['use_mo'] == "on") {
				$for_trarray[$key] = __($for,'booking-calendar');
			} elseif(isset($address['theme_option'][$key])){
				$for_trarray[$key] = $address['theme_option'][$key];
			} else {
				$for_trarray[$key] = __($for,'booking-calendar');
			}
		}
	...
	private function update_reservation($data,$submit){
		...
		$payment_type = (isset($data['payment_type_'.$submit]) ? esc_html(stripslashes( $data['payment_type_'.$submit])) : '');
		$resid = (isset($data['resid_'.$submit]) ? esc_html(stripslashes( $data['resid_'.$submit])) : '');
	...
	private function show_reservation_info($data) {
		...
		if(count($data)) {
			...
			$reserv_info .= self::reservation_item(__('Total','booking-calendar'),(((isset($this->theme_option['currency_pos']) && $this->theme_option['currency_pos'] == "before") ? $data["currency"] : '') . $data["total_price"] . (((isset($this->theme_option['currency_pos']) && $this->theme_option['currency_pos'] == "after") || !isset($this->theme_option['currency_pos'])) ? $data["currency"] : '')));
	...
	public static function wpdevart_payment($id,$type,$addresses,$for_trarray,$resid,$cal_name) {
		...
		$payment_form .= '<input type="hidden" name="resid_'.$id.'" value="'.$resid.'">
		<input type="hidden" name="payment_type_'.$id.'" value="'.$type.'">
		<input type="hidden" name="calendar_name_'.$id.'" value="'.$cal_name.'">
			<input type="hidden" name="view" id="add" value="add"><button type="submit" name="payment_submit_'.$id.'" id="payment_submit_'.$id.'" class="wpdevart-submit order-submit">'.$for_trarray['for_submit_button'].'</button>';
	...
	private static function form_field_countries($form_field,$type){
		$select_options = wpdevart_bc_BookingCalendar::get_countries();
		$input_class = '';
		$field_html = '';
		$field_html .= '<div class="wpdevart-fild-item-container">
							<label for="wpdevart_'.$form_field['name'].'">'.esc_html($form_field['label']).'</label>';

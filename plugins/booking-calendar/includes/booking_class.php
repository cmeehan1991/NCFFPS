...
class wpdevart_bc_BookingCalendar {
		...
		if(isset($this->form_data)) {
			...
			if(isset($this->res)){
				...
				if(isset($this->res["total_price"])){
					$form_html .= '<div class="wpdevart-total-price reserv_info_row"><span class="reserv_info_cell">'.$this->for_tr["for_total"].'</span><span class="reserv_info_cell_value total_price"><span>'.$this->res["total_price"].'</span>'.$this->currency.'</span></div>';
				}
				$form_html .= '</div>';
			} else {
	...
	public function save_reserv($data,$submit,$type = ""){
		...
		$billing_form = array();
		$shipping_form = array();
		$resstatus = (isset($data['reserv_status']) ? esc_html(stripslashes( $data['reserv_status'])) : '');
		$resid = (isset($data['reserv_id']) ? esc_html(stripslashes( $data['reserv_id'])) : 0);
	...
	private function send_mail($emails,$form_data,$extras_data,$count_item,$price,$currency,$total_price,$extras_price,$check_in,$check_out,$single_day,$start_hour, $end_hour){
		...
		if($total_price != "NaN"){
			$res_info .= "<tr><td style='padding: 1px 7px;'>".__('Total Price','booking-calendar')."</td> <td style='padding: 1px 7px;'>".((isset($this->theme_option['currency_pos']) && $this->theme_option['currency_pos'] == "before") ? esc_html($currency) : '') . $total_price . (((isset($this->theme_option['currency_pos']) && $this->theme_option['currency_pos'] == "after") || !isset($this->theme_option['currency_pos'])) ? esc_html($currency) : '')."</td></tr>";
		}
		$res_info .= "</table>";
		$form = "";
		$extras = "";		
		...
		if(count($user_email_types)) {	
			foreach($user_email_types as $user_email_type) {	
				...
				if(isset($this->theme_option['use_phpmailer']) && $this->theme_option['use_phpmailer'] == "on"){
					...
					$mail->AltBody = wp_strip_all_tags($content);
					$mail_to_send->IsHTML(true);
					$mail_to_send->AddAddress($to);
					if ($mail_to_send->Send() ) {

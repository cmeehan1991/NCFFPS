			...
			<table class="wp-list-table widefat fixed pages wpdevart-table wpdevart-reservations-table"> 
				...
				if(isset($_SESSION["clendar_id"]) && $_SESSION["clendar_id"] != 0) {
					foreach ( $rows as $row ) {
						...
						if($row->address_billing && isset($theme_options["billing_address_form"])){
							$billing_data = $this->model_obj->get_form_data($row->address_billing,0,$theme_options["billing_address_form"],"billing_info_");
						}
						if($row->address_shipping && isset($theme_options["shipping_address_form"])){
						...
						<tr id="id_<?php echo $row->id; ?>">
							...
							<div class="reserv-info div-for-clear">
								<div class='reserv-info-container'>
									...
									<span class='form_info'><span class='form_label'><?php _e('Total Price','booking-calendar'); ?></span> <span class='form_value'><?php echo (((isset($theme_options['currency_pos']) && $theme_options['currency_pos'] == "before") ? $row->currency : '') . $row->total_price . (((isset($theme_options['currency_pos']) && $theme_options['currency_pos'] == "after") || !isset($theme_options['currency_pos'])) ? $row->currency : '')); ?></span></span>
								</div>
						...
						<tr>   
							...
							<?php if(WPDEVART_PRO === true) : ?>
								<span class="buttons">
									<a  class="action-button cal-edit-button wpdevart-button"><?php _e('Edit','booking-calendar'); ?><span>(pro)</span></a>	
								  <?php if(isset($row->pay_status) && $row->pay_status != "") :?>
									<a  onclick="quick_edit(this,'<?php echo $row->id; ?>');"  class="action-button quick-edit-button wpdevart-button"><?php _e('Quick Edit','booking-calendar'); ?></a>
								  <?php endif; ?>

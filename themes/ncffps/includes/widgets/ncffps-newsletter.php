<?php
class ncffps_newsletter_widget extends WP_Widget{
	
	function __construct(){
		parent::__construct(
			
			// Base ID of widget
			'ncffps_newsletter_widget',
			
			// Widget name that will appear in the UI
			__(get_bloginfo('title') . ' Newsletters', BLOG_TEXTDOMAIN),
			
			// Widget Description
			array('description' => __('This is where your newsletters will be displayed.', BLOG_TEXTDOMAIN))
		);
	}
	...
	public function widget($args, $instance){
		...
		<li>
			<h3 class="excerpt-title"><?php _e(get_the_title(), BLOG_TEXTDOMAIN);?> <a class="excerpt-link" href="<?php echo the_permalink();?>">Read More</a></h3>
		</li>
...
function ncffps_load_newsletter_widget(){
	register_widget('ncffps_newsletter_widget');
}
add_action('widgets_init', 'ncffps_load_newsletter_widget');

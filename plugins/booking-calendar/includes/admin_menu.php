...
class wpdevart_bc_admin_menu{
	...
	public function create_menu(){
		...
		$wpdevart_pages = array('calendar_page','reservation_page','form_page','extra_page','theme_page');
		...
		$page_manage = add_submenu_page( 'wpdevart-calendars', 'User permissions', 'User permissions<span class="pro_feature" style="font-size: 12px;"> (Pro Feature!)</span>', 'manage_options', 'wpdevart-user-permissions', array($this, 'themes_user_permissions'));
	...
	public function uninstall_booking() {
		...
		if(isset( $_POST['uninstall_booking_data'] )   && wp_verify_nonce( $_POST['uninstall_booking_data'], 'uninstall_booking')){
			...
			<div class="wrap">
			  ...
			  <p><strong><a href="<?php echo wp_nonce_url('plugins.php?action=deactivate&amp;plugin=booking-calendar/booking_calendar.php', 'deactivate-plugin_booking-calendar/booking_calendar.php'); ?>"><?php _e('Click Here','booking-calendar'); ?></a><?php _e(' To Finish the Uninstallation','booking-calendar'); ?></strong></p>
			</div>
		  <?php
			return;
		}
